...
- Unless the user specifies the web/screen edition, treat requests as referring to the print edition (`?edition=print` / `AppPrint`).
...
- When updating print-edition copy, apply text exactly from the user's print markdown handout; do not rewrite prose or make editorial or aesthetic choices unless asked.
...
- Screen `.cv-content-grid` in `App.styles.ts` is asymmetric (`minmax(18rem, 27rem)` sidebar / `1fr` main). Default route uses `App`; `?edition=print` renders `AppPrint` with `PRINT_CONTENT`. `AppPrint` `useEffect` adds `pe-edition` (`html` `font-size: 65%` screen preview) and `pe-full` (`62%` when printing) on `<html>`; `@media print` uses `60%`. Print layout in `src/styles/print.styles.ts` (`printEditionStyles`): 50/50 grid; base rules outside `@media print` for on-screen print preview, `@media print` for PDF overrides (duplicate selectors intentional). Keep gold/Raleway via `.cv-accent` / `var(--colors-primary)` with `print-color-adjust: exact`, not forced black.
