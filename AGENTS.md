...
- Prefer shared CV components with a `view` prop (`full` | `condensed`) over duplicate web/print layout components; use `?view=condensed` for the handout (`?edition=print` is legacy only).
...
- Screen `.cv-content-grid` in `App.styles.ts` is asymmetric (`minmax(18rem, 27rem)` sidebar / `1fr` main). Default `?view=full` uses `App` + `CONTENT`; `?view=condensed` uses `AppPrint` + `PRINT_CONTENT` (`parseCvViewParam` in `src/types/cv-view.types.ts`; legacy `?edition=print` maps to condensed). `ViewNav` is a fixed top-right bar: `SwitchDS` toggles condensed on/off and a **PDF** button runs `window.print()` on the active view. Full-view PDF uses `src/styles/print.styles.ts` (`stylesPrint`, `@media print` on `.cv-*`). Condensed uses `src/styles/condensed.styles.ts` (`condensedViewStyles`): one ruleset for screen and paper; `@media not print` sets `.cv-condensed-paper` to **320mm** so browser layout matches Save as PDF; `AppPrint` sets `html[data-cv-view='condensed']`, which scales type at **62%** in `@media print` only. Condensed DOM uses `cv-condensed-*` classes (not `pe-*`). Tune column gap and body/pill sizes via `PRINT_CONTENT_COL_GAP`, `PRINT_BODY_FONT_SIZE`, and `PRINT_TECH_PILL_FONT_SIZE` in `condensed.styles.ts`. Keep gold/Raleway via `.cv-accent` / `var(--colors-primary)` with `print-color-adjust: exact`, not forced black.
- `CVPrintHeader` (`layout/print/`) mirrors `CVHeader` markup and composes web header styles; QR is the third grid column. Mask condensed avatar grey edge with white `outline` + zero-blur `box-shadow` on `[data-scope='avatar']`. `CVSection` requires `sectionKey` (full view); condensed uses `PrintSection` with `.cv-condensed-section-heading` from `section-heading.styles.ts`. Shared `ProjectEntry` accepts `view`; condensed inlines meta on the title row. Project meta is `vX.X.X · N commits` (or `status` when no version) via `formatProjectMeta`.
